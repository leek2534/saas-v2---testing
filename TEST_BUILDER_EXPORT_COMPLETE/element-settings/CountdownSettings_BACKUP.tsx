...
import { 
  Clock, AlignLeft, AlignCenter, AlignRight, AlignJustify,
  Palette, ChevronDown, ChevronUp, Image as ImageIcon,
  Sparkles, Eye, EyeOff, Type, Wand2
} from 'lucide-react';
import { HexColorPicker } from 'react-colorful';
import { cn } from '@/lib/utils';
import { GradientPicker, GradientValue } from '../GradientPicker';
import { IconPicker } from '../IconPicker';
import { CountdownPresetPicker, COUNTDOWN_PRESETS } from '../CountdownPresets';
...
export function CountdownSettings({ node, updateProps }: CountdownSettingsProps) {
  ...
  const { setRightSidebarExpanded } = useTestBuilderV2Store();
  const [expandedSections, setExpandedSections] = useState<string[]>([
    'presets', 'content', 'padding', 'typography', 'decoration', 'alignment', 'background', 'textShadow', 'icons'
  ]);
  const [showPresetPicker, setShowPresetPicker] = useState(false);

  // Expand/collapse sidebar when preset picker is shown/hidden
  useEffect(() => {
    setRightSidebarExpanded(showPresetPicker);
  }, [showPresetPicker, setRightSidebarExpanded]);
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700">
        <SectionHeader id="presets" title="Style Presets" icon={Wand2} />
        {expandedSections.includes('presets') && (
          <div className="p-4 border-t border-gray-200 dark:border-gray-700">
            <div className="mb-3">
              <p className="text-xs text-gray-600 dark:text-gray-400 mb-3">
                Choose from professionally designed countdown styles. Browse through categories and preview each style before applying.
              </p>
              <Button
                onClick={() => setShowPresetPicker(!showPresetPicker)}
                variant={showPresetPicker ? "secondary" : "default"}
                className="w-full"
                size="sm"
              >
                <Wand2 size={14} className="mr-2" />
                {showPresetPicker ? 'Hide Style Browser' : 'Browse Countdown Styles'}
              </Button>
            </div>

            {showPresetPicker && (
              <div className="mt-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                <CountdownPresetPicker
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs font-semibold text-blue-900 dark:text-blue-100">
                      Current Style: {COUNTDOWN_PRESETS.find(p => p.id === props.presetId)?.name || 'Custom'}
                    </p>
                    <p className="text-[10px] text-blue-700 dark:text-blue-300 mt-0.5">
                      {COUNTDOWN_PRESETS.find(p => p.id === props.presetId)?.description}
                    </p>
                  </div>
                  <Button
      ...
      {/* Content Section */}
      <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700">
        <SectionHeader id="content" title="Content" icon={Clock} />
        {expandedSections.includes('content') && (
          <div className="p-4 space-y-4 border-t border-gray-200 dark:border-gray-700">
            <div>
              <Label className="text-xs text-gray-600 dark:text-gray-400 mb-2 block">Countdown Title</Label>
              <Input
                value={props.title || ''}
            ...
            <div>
              <Label className="text-xs text-gray-600 dark:text-gray-400 mb-2 block">Countdown Type</Label>
            ...
            {props.countdownType === 'fixed' && (
              <div>
                <Label className="text-xs text-gray-600 dark:text-gray-400 mb-2 block">End Date & Time</Label>
                <Input
                  type="datetime-local"
                  value={props.endDate || ''}
            ...
            {props.countdownType === 'evergreen' && (
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <Label className="text-xs text-gray-600 dark:text-gray-400 mb-2 block">Days</Label>
                  <Input
                    type="number"
                    value={props.evergreenDays || 0}
                    onChange={(e) => updateProps({ evergreenDays: parseInt(e.target.value) })}
                    min={0}
                  />
                </div>
                <div>
                  <Label className="text-xs text-gray-600 dark:text-gray-400 mb-2 block">Hours</Label>
                  <Input
                    type="number"
                    value={props.evergreenHours || 24}
                    onChange={(e) => updateProps({ evergreenHours: parseInt(e.target.value) })}
                    min={0}
                    max={23}
                  />
                </div>
            ...
            <div>
              <Label className="text-xs text-gray-600 dark:text-gray-400 mb-2 block">Display Format</Label>
            ...
            <div className="space-y-2">
              <Label className="text-xs text-gray-600 dark:text-gray-400">Unit Labels</Label>
              <div className="grid grid-cols-2 gap-2">
                <Input
                  value={props.daysLabel || 'Days'}
                  onChange={(e) => updateProps({ daysLabel: e.target.value })}
                  placeholder="Days"
                />
                <Input
                  value={props.hoursLabel || 'Hours'}
                  onChange={(e) => updateProps({ hoursLabel: e.target.value })}
                  placeholder="Hours"
                />
                <Input
                  value={props.minutesLabel || 'Minutes'}
                  onChange={(e) => updateProps({ minutesLabel: e.target.value })}
                  placeholder="Minutes"
                />
                <Input
                  value={props.secondsLabel || 'Seconds'}
                  onChange={(e) => updateProps({ secondsLabel: e.target.value })}
                  placeholder="Seconds"
                />
              </div>
            </div>
            ...
            <div>
              <Label className="text-xs text-gray-600 dark:text-gray-400 mb-2 block">Expired Message</Label>
              <Input
                value={props.expiredMessage || ''}
                onChange={(e) => updateProps({ expiredMessage: e.target.value })}
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <Label className="text-xs text-gray-600 dark:text-gray-400">Redirect on Expire</Label>
                <Switch
                  checked={props.redirectOnExpire || false}
                  onCheckedChange={(checked) => updateProps({ redirectOnExpire: checked })}
                />
              </div>

              {props.redirectOnExpire && (
                <Input
                  value={props.expireRedirectUrl || ''}
                  onChange={(e) => updateProps({ expireRedirectUrl: e.target.value })}
