...
export async function defaultToAccessTeamSlug(viewer: Ent<"users">) {
  const teams = await viewer.edge("members").map((member) => member.edge("team").doc());
  const personalTeam = teams.filter((team) => team.deletionTime === undefined && team.isPersonal)[0];
  if (!personalTeam) {
    throw new Error("No personal team found for user. Please contact support.");
  }
  return personalTeam.slug;
}
