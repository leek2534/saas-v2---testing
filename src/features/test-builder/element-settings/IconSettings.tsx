...
import { 
  Star, AlignLeft, AlignCenter, AlignRight, AlignJustify,
  Palette, ChevronDown, ChevronUp, Image as ImageIcon,
  Sparkles, Eye, EyeOff, Type, Wand2, Search
} from 'lucide-react';
...
const ICON_PRESETS = [
  {
    id: 'social-large',
    name: 'Social Large',
    description: 'Large social media icons',
    props: {
      icon: 'facebook',
      size: 48,
      color: '#1877f2',
  ...
  },
  {
    id: 'arrow-cta',
    name: 'Arrow CTA',
    description: 'Call-to-action arrow',
    props: {
      icon: 'arrow-right',
      size: 20,
      color: '#3b82f6',
...
export function IconSettings({ node, updateProps }: IconSettingsProps) {
  ...
  return (
    <div className="space-y-2">
          ...
          <div className="mt-3 p-2 bg-primary/10 rounded-lg border border-blue-200 dark:border-blue-800">
            <div className="flex items-center justify-between">
              <p className="text-xs font-semibold text-blue-900 dark:text-blue-100">
                Current: {ICON_PRESETS.find(p => p.id === props.presetId)?.name || 'Custom'}
              </p>
      ...
      <div className="bg-card rounded-lg border border-border">
        <SectionHeader id="content" title="Content" icon={Star} />
        {expandedSections.includes('content') && (
          <div className="p-4 space-y-4 border-t border-border">
            {/* Icon Library */}
            <div>
              <Label className="text-xs text-muted-foreground mb-2 block">Icon Library</Label>
              <Select value={props.library || 'lucide'} onValueChange={(value) => updateProps({ library: value })}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="lucide">Lucide Icons (1000+)</SelectItem>
                  <SelectItem value="fontawesome">Font Awesome (10,000+)</SelectItem>
                  <SelectItem value="heroicons">Hero Icons (300+)</SelectItem>
                  <SelectItem value="material">Material Icons (2000+)</SelectItem>
                  <SelectItem value="feather">Feather Icons (280+)</SelectItem>
            ...
            {/* Icon Picker */}
            {props.library !== 'custom' && (
              <IconPicker
                value={props.icon || 'Star'}
                onChange={(iconName: any) => updateProps({ icon: iconName })}
                label="Select Icon"
              />
            )}
            ...
            {props.library === 'custom' && (
              <div>
                <Label className="text-xs text-muted-foreground mb-2 block">Custom SVG Code</Label>
                <textarea
                  value={props.customSvg || ''}
                  onChange={(e) => updateProps({ customSvg: e.target.value })}
                  placeholder="<svg>...</svg>"
            ...
            <div>
              ...
              <Select value={props.iconType || 'solid'} onValueChange={(value) => updateProps({ iconType: value })}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
              ...
              {props.showBackground && (
                <>
                  <div>
                    ...
                    <Select value={props.bgShape || 'circle'} onValueChange={(value) => updateProps({ bgShape: value })}>
                      <SelectTrigger><SelectValue /></SelectTrigger>
                      <SelectContent>
                  ...
                  <ColorPicker
                    ...
                    onChange={(color) => updateProps({ bgColor: color })}
                  />

                  <SliderWithInput
                    label="Background Padding"
                    value={props.bgPadding || 16}
                    onChange={(val) => updateProps({ bgPadding: val })}
                    min={0}
                    max={50}
                    unit="px"
                  />
            ...
            <SliderWithInput
              ...
              onChange={(val) => updateProps({ rotation: val })}
              min={0}
              max={360}
              unit="°"
            />
            ...
            <div>
              ...
              <Select value={props.flip || 'none'} onValueChange={(value) => updateProps({ flip: value })}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="none">None</SelectItem>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <Switch
                  checked={props.hasLink || false}
                  onCheckedChange={(checked) => updateProps({ hasLink: checked })}
                />
              </div>
                ...
                <>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) => updateProps({ linkNewTab: checked })}
                    />
                  </div>
                </>
              )}
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) => updateProps({ showBadge: checked })}
                />
              </div>
              ...
              {props.showBadge && (
                <>
                  ...
                  <div>
                    ...
                    <Select value={props.badgePosition || 'top-right'} onValueChange={(value) => updateProps({ badgePosition: value })}>
                      <SelectTrigger><SelectValue /></SelectTrigger>
                      <SelectContent>
