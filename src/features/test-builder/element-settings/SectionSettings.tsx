...
export function SectionSettings({ node, updateProps }: SectionSettingsProps) {
          ...
          <div className="mt-3 p-2 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
            <div className="flex items-center justify-between">
              <p className="text-xs font-semibold text-blue-900 dark:text-blue-100">
                Current: {SECTION_PRESETS.find(p => p.id === props.presetId)?.name || 'Custom'}
              </p>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <TabsContent value="content" className="p-4 space-y-4 m-0">
            <SectionCard id="container" title="Container Width" icon={Box}>
              <div className="space-y-3">
                <div>
                  <Label className="text-xs">Container Type</Label>
                  <Select value={props.containerType || 'boxed'} onValueChange={(value) => updateProps({ containerType: value })}>
                    <SelectTrigger className="mt-1">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="full">Full Width</SelectItem>
                      <SelectItem value="boxed">Boxed (1200px)</SelectItem>
                      <SelectItem value="narrow">Narrow (960px)</SelectItem>
                      <SelectItem value="wide">Wide (1400px)</SelectItem>
                    </SelectContent>
                  </Select>
                  <p className="text-xs text-foreground mt-1">Maximum content width</p>
                </div>
              </div>
            </SectionCard>
